...
operation_result gcm_setkey(void* ctx,
	...
	if (NULL == ctx || NULL == key || (keybits != 128 && keybits != 192 && keybits != 256)) {
		return OPERATION_FAIL;
	}
	gcm_context* temp_ctx = (gcm_context*)ctx;
	if (AES_set_encrypt_key(key, keybits, &temp_ctx->aes_key) != 0) {
		return OPERATION_FAIL;
	}
...
operation_result gcm_crypt_and_tag(void* context,
	...
	// set H
	AES_encrypt(y0, ency0, &ctx->aes_key);
	...
	for (i = 0; i < length / GCM_BLOCK_SIZE; ++i) {
        ...
        printf_output(y0, GCM_BLOCK_SIZE);
#endif 
		AES_encrypt(y0, ctx->buff, &ctx->aes_key);
...
operation_result aes256_gcm_enc (const unsigned char *key,
	const unsigned char *iv,
    size_t iv_len,
    const unsigned char *add,
    size_t add_len,
    ...
    size_t tag_len) 
{
	...
	operation_result flag = gcm_setkey(context, key, 256 );
...
operation_result aes256_gcm_dec (const unsigned char *key,
	const unsigned char *iv,
    size_t iv_len,
    const unsigned char *add,
    size_t add_len,
