...
void vect_mul(uint64_t *o, const uint64_t *v1, const uint64_t *v2)
    {
        ...
        unsigned long long c[VEC_N_SIZE_64 * 2];
        memset(c,0,sizeof(c));
        gf2x_mul((unsigned long *)c, (unsigned long *)v1, VEC_N_SIZE_64, (unsigned long *)v2, VEC_N_SIZE_64);

        for (int i = PARAM_N; i + 1 < 2 * PARAM_N; i++)
        {
            if (c[i / 64] & (1ULL << (i % 64)))
            {
                int j = (i - PARAM_N);
        ...
        memcpy(o, c, VEC_N_SIZE_64*sizeof(unsigned long long));
    }
