...
void vect_set_random_from_prng(uint64_t *v, uint32_t size_v) {
    uint8_t rand_bytes [64] = {0}; // set to the maximum possible size - 256 bits

    
    shake_prng(rand_bytes, min(size_v << 3, 32));
    shake_prng(rand_bytes + 32, min(size_v << 3, 32));
 ...
 *
 * Function borrowed from liboqs: https://github.cppom/open-quantum-safe/liboqs/commit/cce1bfde4e52c524b087b9687020d283fbde0f24
 *
